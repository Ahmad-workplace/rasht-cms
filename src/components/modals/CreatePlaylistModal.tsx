...
interface CreatePlaylistModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (data: Omit<PlaylistDetail, 'id'>) => void;
}
...
const CreatePlaylistModal: React.FC<CreatePlaylistModalProps> = ({
  ...
  const [draftPlaylist, setDraftPlaylist] = useState<Omit<PlaylistDetail, 'id'>>({
    title: 'پلی‌لیست جدید',
    is_active: true,
    schedules: []
  });
  ...
  const handleSavePlaylist = async () => {
    try {
      ...
      await onSubmit(playlistToSave);
      toast.success('پلی‌لیست با موفقیت ایجاد شد');
      onClose();
    } catch (error) {
      toast.error('ایجاد پلی‌لیست با خطا مواجه شد');
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto">
      <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div className="inline-block w-full max-w-6xl px-4 pt-5 pb-4 overflow-hidden text-right align-bottom transition-all transform bg-white rounded-lg shadow-xl sm:my-8 sm:align-middle sm:p-6 max-h-[90vh] flex flex-col">
          <DragDropContext onDragStart={handleDragStart} onDragEnd={handleDragEnd} onDragUpdate={handleDragOver}>
            <div className="flex flex-1 overflow-hidden">
              <div className="flex-1 px-1.5 py-3 overflow-y-auto">
